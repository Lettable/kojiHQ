  ...
  return (
    <div className={`min-h-screen ${isDarkTheme ? 'bg-black text-white' : 'bg-white text-black'}`}>
      <div className="container mx-auto px-4 py-8">
        <Card className={`w-full max-w-4xl mx-auto ${isDarkTheme ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-2xl font-bold">Notifications</CardTitle>
            <Badge variant="secondary" className={isDarkTheme ? 'bg-yellow-400/20 text-yellow-400' : 'bg-yellow-100 text-yellow-800'}>
              {unreadCount} Unread
            </Badge>
          </CardHeader>
          <CardContent>
              ...
              <AnimatePresence>
                {filteredNotifications.map((notification) => (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: -20 }}
                    transition={{ duration: 0.2 }}
                    className={`p-4 border-b ${isDarkTheme ? 'border-zinc-800' : 'border-zinc-200'} ${
                      !notification.read ? (isDarkTheme ? 'bg-yellow-400/10' : 'bg-yellow-50') : ''
                    }`}
                  >
